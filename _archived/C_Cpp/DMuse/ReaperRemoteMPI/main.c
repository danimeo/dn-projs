...
void PushMsg(int chan, char *msg, int n) {
	int i, n2, j = 0;
	n2 = n + 4;
	for (i = -4; i < n2; i++) {
		if (i < 0) {
...
void Listen(char *send_tagname, char *recv_tagname, int m_size, int n_chan) {
	...
	for (i = 0; i < n_channels; i++) {
		...
		*(addr_recv + i) = (char *) MapViewOfFile(*(hMap_send + i), FILE_MAP_ALL_ACCESS, 0, 0, mem_size);
...
int main() {
	Listen("rpr_r2c_", "rpr_c2r_", 1024, 10);
	int i;
	char s[1024], t[1024], buff[8];
	for (i = 0; i < 10; i++) {
		strcpy(s, "Why??");
		itoa(i, buff, 10);
		strcat(s, buff);
