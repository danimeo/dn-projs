...
cwd = r'D:\git_repos\personal-notes-1\codes\DLearn\pydlearn\dcam1'
...
task_log_filename = cwd + '/dcam_data/records/dcam_timer_log.txt'
task_records_filename = cwd + '/dcam_data/records/dcam_timer_records_2023a.txt'
notes_filename = cwd + '/notes/notes_2023a.txt'
auto_jump_to_task_0 = True
auto_jump_to_undone_task = True
receiving_bci_data = False

auto_start_time = '2023-02-14 12:15:00'
timer_event_name, timer_event_type = '复习', '课内 & 复习'
task_list = [
    ('(自由时间)', '', 10),
    ('J', '控制工程基础', 15),
    ('K', '控制工程基础', 15),
...
stopKey = [w.strip() for w in codecs.open(cwd + '/data/stopWord.txt', 'r', encoding='utf-8').readlines()]
temp = pseg.lcut('测试文本')
...
def write_to_records_file():
    ...
    with open(task_records_filename, 'r+', encoding='utf-8') as file_io:
        ...
        '''if not lines or not lines[-1]:
            ...
            return'''
        if lines and lines[-1] and datetime.datetime.strptime(lines[-1].split('\t')[0], '%Y-%m-%d %H:%M:%S.%f') == start_time:
            file_io.seek(0)
            file_io.truncate()
