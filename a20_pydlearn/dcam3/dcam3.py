...
class TimeCollection:
    ...
    def add(self, segment_or_collection):
        if isinstance(segment_or_collection, TimeSegment):
            ...
            self.segments.clear()
            self.segments.extend(sum_list)
        elif isinstance(segment_or_collection, TimeCollection):
            for segment in segment_or_collection.segments:
                self.add(segment)

    def clear(self):
        self.segments.clear()

    def get_segments_between(self, segment1: TimeSegment, segment2: TimeSegment):
        between = []
        for segment in self.segments:
...
class Task:
    ...
    def pause(self):
        if self.duration.pointer:
            frag = TimeSegment(self.duration.pointer, now(), self.code)
    ...
    def print(self):
        ...
        segments = sorted(self.duration.segments)
        for segment in segments:
            print('  [' + str(segment) + ']')
        if self.duration.pointer:
            print('  [' + str(TimeSegment(self.duration.pointer, now(), self.code)) + ']')
...
class TimeTrack:
    ...
    def add(self, item):
        cont = TimeCollection()
        cont.segments = self.content.segments.copy()
        if isinstance(item, Task):
    ...
    def utilization_ratio(self, purpose=''):
        if purpose:
            ...
            return self.content.get_used_duration() / self.capacity.get_duration()
